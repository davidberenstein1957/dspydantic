...
import OptimizationRunFilters from './OptimizationRunFilters'
import StartOptimizationModal from './StartOptimizationModal'

export default function OptimizationRunsTableView({ taskId, task }) {
  ...
  const { data: runs, isLoading } = useQuery({
    queryKey: ['optimization-runs', taskId],
    queryFn: async () => {
      const response = await optimizationApi.list(taskId)
  ...
  const deleteMutation = useMutation({
    mutationFn: optimizationApi.delete,
    onSuccess: () => {
  ...
  const filteredAndSortedRuns = useMemo(() => {
    ...
    if (filters.optimizer) {
      filtered = filtered.filter(run => run.config?.optimizer === filters.optimizer)
    }
  ...
  const handleDeleteClick = async (run) => {
    setRunToDelete(run)
    try {
      const response = await optimizationApi.getDeleteInfo(run.id)
      setDeleteInfo(response.data)
      setShowDeleteModal(true)
    } catch (error) {
      console.error('Error fetching delete info:', error)
      // Still show delete modal but without delete info
      setDeleteInfo(null)
      setShowDeleteModal(true)
    }
  }
  ...
  if (isLoading) {
    return <div className="text-center py-8">Loading optimization runs...</div>
  }
              ...
              {filteredAndSortedRuns && filteredAndSortedRuns.length > 0 ? (
                filteredAndSortedRuns.map((run) => (
                  <tr
                    ...
                    onClick={() => {
                      // Navigate to a detail view or show details in a modal
                      // For now, we'll just show the error if there is one
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {run.config?.prompt_version_id && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation()
                              navigate(`/tasks/${taskId}`, { state: { activeTab: 'prompts', selectedVersionId: run.config.prompt_version_id } })
                            }}
                            className="text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded hover:bg-blue-100"
                          >
                            Prompt v{run.prompt_version_number ?? run.config.prompt_version_id}
                          </button>
                        )}
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">
                      {run.metrics ? (
                        <div className="text-xs space-y-1">
                          {run.metrics.optimized_score !== undefined && (
                            <div>Score: {(run.metrics.optimized_score * 100).toFixed(2)}%</div>
                          )}
                          {run.metrics.train_score !== undefined && (
                            <div>Train: {(run.metrics.train_score * 100).toFixed(2)}%</div>
                          )}
                          {run.metrics.test_score !== undefined && (
                            <div>Test: {(run.metrics.test_score * 100).toFixed(2)}%</div>
                          )}
                        </div>
                      ) : (
                        '-'
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Delete Optimization Run</h3>
            <div className="mb-4">
              <p className="text-sm text-gray-700 mb-3">
                Are you sure you want to delete this optimization run? This action cannot be undone.
              </p>
              {deleteInfo && (deleteInfo.prompt_versions_count > 0 || deleteInfo.evaluation_runs_count > 0) && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3">
                  <p className="text-sm font-medium text-yellow-800 mb-2">Warning: The following will also be deleted:</p>
                  <ul className="text-sm text-yellow-700 list-disc list-inside space-y-1">
                    {deleteInfo.prompt_versions_count > 0 && (
                      <li>{deleteInfo.prompt_versions_count} prompt version{deleteInfo.prompt_versions_count !== 1 ? 's' : ''} created by this optimization run</li>
                    )}
                    {deleteInfo.evaluation_runs_count > 0 && (
                      <li>{deleteInfo.evaluation_runs_count} evaluation run{deleteInfo.evaluation_runs_count !== 1 ? 's' : ''} associated with those prompt versions</li>
                    )}
                  </ul>
                </div>
              )}
