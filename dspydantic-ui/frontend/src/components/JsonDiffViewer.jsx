...
function diffJson(oldObj, newObj) {
  const oldText = formatJson(oldObj)
  const newText = formatJson(newObj)
...
export default function JsonDiffViewer({ oldValue, newValue, title }) {
  const diff = useMemo(() => diffJson(oldValue, newValue), [oldValue, newValue])
